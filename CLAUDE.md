...
- **Rotas atuais:** `/` (home), `/detalhes/` (programa completo), `/agenda/`, `/planos/conhecer|adesao|dedicacao|aceleracao/` e `/planos/checkout/dedicacao/` (**não existe índice em `/planos/`** — a comparação foi removida em 01/08 e vive na seção `#precos` da home), `/ff/` (Fórmula Fluente), `/conversacao/`, `/curitiba/`, `/online/`, `/imersoes/` (+ `/curitiba/`, `/belo-horizonte/`, `/florida/`), `/blog/` (+ posts e `/blog/indicacoes/<slug>/`), `/politica-privacidade/`, `/termos-uso/`, `/en/` e `/es/` (landings i18n com subrotas próprias). Cada rota é um `app/<rota>/page.tsx` que monta `Navbar` + componentes de seção + `Footer`.
...
- **Checkout: metade ligado.** As duas constantes ficam no topo de `lib/planos.ts`. `CHECKOUT_ADESAO` já é o gateway real — `https://azuris.com.br/ett/adesao`, PIX ou cartão em até 3x, funcionando de ponta a ponta (a prévia interna da adesão foi apagada). `CHECKOUT_DEDICACAO` **continua apontando pra prévia interna** `/planos/checkout/dedicacao/` de propósito: `https://azuris.com.br/ett/assinatura` abre com os preços certos, mas o botão devolve **500** até rodar a migração de produção que cria a coluna `assinaturas.produto_slug`. Assim que a migração rodar, trocar a constante **e apagar `app/planos/checkout/`**. Nunca criar checkout falso que peça cartão: o site está no ar e recebe tráfego real.
