...
- **Rotas atuais:** `/` (home), `/detalhes/` (programa completo), `/agenda/`, `/planos/` (+ `/conhecer/`, `/adesao/`, `/dedicacao/`, `/aceleracao/` e `/checkout/adesao|dedicacao/`), `/ff/` (Fórmula Fluente), `/conversacao/`, `/curitiba/`, `/online/`, `/imersoes/` (+ `/curitiba/`, `/belo-horizonte/`, `/florida/`), `/blog/` (+ posts e `/blog/indicacoes/<slug>/`), `/politica-privacidade/`, `/termos-uso/`, `/en/` e `/es/` (landings i18n com subrotas próprias). Cada rota é um `app/<rota>/page.tsx` que monta `Navbar` + componentes de seção + `Footer`.
...
- **Modelo de cobrança — `webapp/lib/planos.ts` é a fonte única.** Preços, as 2 portas de entrada, as 2 trilhas, os cartões da home, o detalhe de cada plano, os prêmios e o FAQ de cobrança vivem **só** nesse arquivo; `components/Precos.tsx` (home, `#precos`), `components/Planos.tsx` (`/planos/`) e `components/PlanoDetalhe.tsx` (páginas de plano) leem dele. **Não escrever preço à mão em componente.** Escada publicada: **Conhecer** (R$ 0, 30 dias, sem cartão) → **Adesão** (R$ 70, cobrança única, inclui 2×1h de mentoria individual, material didático e os 30 primeiros dias) → **Trilha de Dedicação** (R$ 39/mês) ou **Trilha de Aceleração** (R$ 0, revalidada por 20 dias válidos/mês). Regras de copy que não podem ser quebradas: **nunca publicar "R$ 0" sozinho** (sempre com âncora `~~R$ 39/mês~~` + o custo em outra moeda, "custa 1h da sua dedicação por dia"); o **encontro online de segunda continua aberto a qualquer pessoa, sem cadastro e sem pagar**; nada de preço simbólico (R$ 1/R$ 15 foram analisados e descartados — cheiro de infoproduto). Especificação completa (economia, prêmios, migração, perguntas em aberto) em **`novoConteudo/MODELO-COBRANCA-ETT.md`**.
- **Checkout ainda não existe.** `CHECKOUT_ADESAO` e `CHECKOUT_DEDICACAO`, no topo de `lib/planos.ts`, apontam pras **prévias internas** em `/planos/checkout/*` — que mostram o resumo do pedido com valores reais, avisam que o pagamento online não abriu e mandam concluir pelo WhatsApp. Nunca criar checkout falso que peça cartão: o site está no ar e recebe tráfego real. Quando as URLs do gateway chegarem, trocar as duas constantes **e apagar `app/planos/checkout/`**.
