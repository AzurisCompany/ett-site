...
- **`/agenda/` também é calculada no navegador.** `webapp/lib/agenda-events.ts` exporta `agendaEvents` (eventos datados, escritos à mão) + `encontrosOnlineRecorrentes()` (as segundas, geradas por regra) + `eventosFuturos(semanas)`, que mescla os dois e descarta o que já passou; `Agenda.tsx` chama isso em `useEffect` e mostra skeleton antes. Foi assim que se resolveu a agenda congelada em "11 de maio a 4 de junho" (27/07/2026) — **não voltar a renderizar `agendaEvents` direto nem escrever o período do título à mão.** Evento pode ter `links: [{url,label}]` (sala, grupo) e `partner` (selo do parceiro). Encontro presencial continua sendo cadastro manual.
